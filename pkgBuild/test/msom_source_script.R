...
# nohup R CMD BATCH -cwd --no-save trawlDiversity/pkgBuild/test/msom_source_script.R msom_source_script_AllRegs_annual_stan_2kIter_r9-10.Rout &
...
for(r in 9:10){ # shelf and newf
	...
	for(i in 1:length(u_yrs)){
		...
		rm_out[[r]][[i]] <- tryCatch(run_msom(
			...
			cores = 4, chains = 4,
			test=FALSE, n0=annual_n0[i], iter=2000, pre_save=FALSE, save_warmup=FALSE
